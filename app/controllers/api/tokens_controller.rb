  ...
  class TokensController < Api::BaseController
    ...
    # Grab the required params
      ...
      if @user.nil? # If user does not exist
        render :status=>401, :state => 'error', :json=>{:message=>"Invalid email or passoword."}
        return
      end
      ...
      if not @user.valid_password?(password) # Check the password
        render :status=>401, :state => 'error', :json=>{:message=>"Invalid email or password."}

      else
        render :status=>200, :state => 'ok', :json=> {:token=>@user.authentication_token, :user=>@user}
    ...
    def destroy
      @user=User.find_by_authentication_token(params[:id])
      if @user.nil?
        render :status=>404, :state => 'error', :json=>{:message=>"Invalid token."}
      else
        @user.reset_authentication_token!
        render :status=>200, :state => 'ok', :json=>{:token=>params[:id]}
      end
    end
